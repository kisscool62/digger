...
class MonsterManager {
    ...
    private void monai(int mon) {
        ...
        if ((clbits & dig.getBagManager().bagbits()) != 0) {
            mondat[mon].increaseT(); /* Time penalty */
            mongotgold = false;
            if (mondat[mon].getDir() == 4 || mondat[mon].getDir() == 0) { /* Horizontal push */
                push = dig.getBagManager().pushbags(mondat[mon].getDir(), clbits);
                mondat[mon].increaseT(); /* Time penalty */
            } else if (!dig.getBagManager().pushudbags(clbits)) /* Vertical push */
                push = false;
            if (mongotgold) /* No time penalty if monster eats gold */
                mondat[mon].setT(0);
            if (!mondat[mon].isNob() && mondat[mon].getHnt() > 1)
                dig.getBagManager().removebags(clbits); /* Hobbins eat bags */
        }
    ...
    private void mondie(int mon) {
        switch (mondat[mon].getDeath()) {
            case 1:
                if (dig.getBagManager().bagy(mondat[mon].getBag()) + 6 > mondat[mon].getY())
                    mondat[mon].setY(dig.getBagManager().bagy(mondat[mon].getBag()));
                dig.getDrawing().drawmondie(mon, mondat[mon].isNob(), mondat[mon].getHdir(), mondat[mon].getX(), mondat[mon].getY());
                dig.getMain().increasePenalty();
                if (dig.getBagManager().getbagdir(mondat[mon].getBag()) == -1) {
    ...
    protected void squashmonsters(int bag, int bits) {
        ...
        for (m = 0, b = 256; m < 6; m++, b <<= 1)
            if ((bits & b) != 0)
                if (mondat[m].getY() >= dig.getBagManager().bagy(bag))
