...
class Sprite {
    ...
    protected void createspr(int n, int ch, short[] mov, int wid, int hei, int bwid, int bhei) {
    ...
    protected void drawmiscspr(int x, int y, int ch, int wid, int hei) {
        sprx[16] = x & -4;
        spry[16] = y;
        sprch[16] = ch;
        sprwid[16] = wid;
        sprhei[16] = hei;
        dig.getPc().gputim(sprx[16], spry[16], sprch[16], sprwid[16], sprhei[16]);
    }

    protected int drawspr(int n, int x, int y) {
        int bx, t1, t2, t3, t4;
        bx = n & 15;
        x &= -4;
        clearrdrwf();
        ...
        dig.getPc().ggeti(sprx[bx], spry[bx], sprmov[bx], sprwid[bx], sprhei[bx]);
        putims();
        return bcollides(bx);
    }

    protected void erasespr(int n) {
        int bx = n & 15;
        dig.getPc().gputi(sprx[bx], spry[bx], sprmov[bx], sprwid[bx], sprhei[bx], true);
        sprenf[bx] = false;
        clearrdrwf();
        setrdrwflgs(bx);
        putims();
    ...
    protected void getis() {
        ...
        for (i = 0; i < 16; i++)
            if (sprrdrwf[i])
                dig.getPc().ggeti(sprx[i], spry[i], sprmov[i], sprwid[i], sprhei[i]);
        putims();
    }

    protected void initmiscspr(int x, int y, int wid, int hei) {
        sprx[16] = x;
        spry[16] = y;
        sprwid[16] = wid;
        sprhei[16] = hei;
    ...
    protected void initspr(int n, int ch, int wid, int hei, int bwid, int bhei) {
        sprnch[n & 15] = ch;
        sprnwid[n & 15] = wid;
        sprnhei[n & 15] = hei;
        sprnbwid[n & 15] = bwid;
    ...
    protected int movedrawspr(int n, int x, int y) {
        int bx = n & 15;
        sprx[bx] = x & -4;
        spry[bx] = y;
        sprch[bx] = sprnch[bx];
    ...
    private void putims() {
        ...
        for (i = 0; i < 16; i++) {
            j = sprorder[i];
            if (sprrdrwf[j])
                dig.getPc().gputim(sprx[j], spry[j], sprch[j], sprwid[j], sprhei[j]);
        }
    }
    ...
    private void putis() {
        ...
        for (i = 0; i < 16; i++)
            if (sprrdrwf[i])
                dig.getPc().gputi(sprx[i], spry[i], sprmov[i], sprwid[i], sprhei[i]);
    }
    ...
    protected void setretr(boolean f) {
        retrflag = f;
    }

    protected void setsprorder(int[] newsprorder) {
        if (newsprorder == null)
